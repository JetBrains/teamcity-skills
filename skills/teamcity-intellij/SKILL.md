...
All necessary TeamCity skills are already available to you, simply read them.
Use `teamcity-cli` (or another skill that fits the task) and read its
reference files instead of guessing:
...
**If one side is missing:** no MCP server configured → do everything through the CLI. No usable CLI → you are limited
to MCP.
...
1. **Find the server.** `auth status --no-input` prints the servers the CLI knows, marks the default one and shows each
   token with its expiry. Prefer the server this repository is linked to (`teamcity.toml`). Ask the user if
   there is no link and no default. Say in your answer which server you used, and keep one task on one server.
