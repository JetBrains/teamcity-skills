...
- **Android job:** use the Gradle runner where the Pipeline schema supports it.
  ...
  the build output.
- **iOS job:** set `jobs.<ios-job-id>.runs-on` to the exact macOS/Xcode
  agent/image value offered by the target server, then read back the YAML to
  verify it. Do not leave the job with only a generic macOS requirement or use
  a transient VM name. Reuse a checked-in Fastlane setup when it already
...
Before queueing, verify that the selected TeamCity server exposes a compatible
Linux/Android environment and a macOS/Xcode environment when those jobs are
generated. For each platform-specific job, set the server-offered value in
`jobs.<job-id>.runs-on`, then read the saved YAML back to verify it was
applied. Do not pin a transient cloud-VM name or reuse an agent choice from
another server. An iOS job must not be sent to a generic Linux agent.
